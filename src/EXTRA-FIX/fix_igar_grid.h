...
#ifdef FIX_CLASS
// clang-format off
FixStyle(igar/grid,FixIGARGrid);
// clang-format on
#else

#ifndef LMP_FIX_IGAR_GRID_H
#define LMP_FIX_IGAR_GRID_H

#include "fix_igar.h"

namespace LAMMPS_NS {

class FixIGARGrid : public FixIGAR {
 public:
  FixIGARGrid(class LAMMPS *, int, char **);
  ~FixIGARGrid() override;
  ...
  double interpolation_igar_grid_new(double, double, double);
 ...
 private:
  ...
  void read_igar_energies(const std::string &);
